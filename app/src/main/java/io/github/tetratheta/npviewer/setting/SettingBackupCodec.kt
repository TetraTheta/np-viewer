...
object SettingBackupCodec {
  ...
  private val swipeFractions = setOf(
    "0.05",
    "0.10",
    "0.15",
    "0.20",
  ...
  fun export(context: Context, appVersion: String, exportedAt: String, bookmarks: List<BookmarkItem>): String {
  ...
  fun startPageKeyToUrl(key: String): String = when (key) {
  ...
  fun startPageUrlToKey(url: String): String = when (url) {
  ...
  private fun parseSettings(json: JSONObject): BackupSettings = BackupSettings(
  ...
  private fun parseBookmarks(array: JSONArray): List<BookmarkItem> = buildList {
  ...
  private fun BackupSettings.toJson(): JSONObject = JSONObject()
  ...
  private fun JSONObject.requireString(key: String, allowedValues: Set<String>): String {
  ...
  private fun validOrDefault(value: String?, allowedValues: Set<String>, defaultValue: String): String =
    if (value in allowedValues) value.orEmpty() else defaultValue
}
